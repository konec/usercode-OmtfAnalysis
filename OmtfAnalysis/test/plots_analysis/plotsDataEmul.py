...
def cDataEmulIssue(canvas):
  ...
  t.SetTextSize(0.03)
  t.SetTextColor(2)
  t.DrawLatex(0.55, 0.905,"inclusive (duplicative) cathegories")
  c.Update()
  return
...
#  h.SetMinimum(1.e-4)
  ...
  d.SetTextSize(0.02)
  d.SetTextAngle(90)
  for bin in range(1,hD.GetNbinsX()+1) :
    delta=hE.GetBinContent(bin)-hD.GetBinContent(bin)
    print ('bin: ',bin,' center: ',hD.GetBinCenter(bin), 'diff: ', delta)
    xpos = hD.GetBinCenter(bin)
    ypos = hD.GetBinContent(bin)
    diff="#Delta_{E-D} "+"= {0:n}".format(delta)
    d.DrawLatex(xpos+0.3,ypos,diff)
  c.Update()
  ...
  t.DrawLatex(0.4, ypos+0.05,"exclusive (progressive) cathegories")
  t.DrawLatex(xpos, ypos, aver)
  c.Update()
  return
